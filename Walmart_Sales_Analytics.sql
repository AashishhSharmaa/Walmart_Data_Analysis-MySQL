...
SELECT city AS city, branch, SUM(total) AS total_revenue
FROM sales
...
SELECT branch AS branch, SUM(quantity) AS qty
FROM sales
GROUP BY branch
...
SELECT product_line AS product_line, ROUND(AVG(rating), 2) AS rating
FROM sales
GROUP BY product_line
...
SELECT gender, COUNT(customer_type) AS customer_count
FROM sales
GROUP BY gender
ORDER BY customer_count DESC;
...
-- "A", "B","C" will give branch ratings
